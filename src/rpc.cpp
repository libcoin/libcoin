...
Value getaddresstxes(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
                            "getaddresstxes <btcaddr>\n"
                            "Get transactions involving <btcaddr>");
    uint160 hash160 = CBitcoinAddress(params[0].get_str()).GetHash160();
    ...
    txdb.ReadBtcIndex(hash160, txhashes);
    
    Array list;
    
    for (set<uint256>::const_iterator it = txhashes.begin(); it != txhashes.end(); ++it) {
        list.push_back(Value((*it).ToString()));
    }
...
pair<string, rpcfn_type> pCallTable[] =
{
    ...
    make_pair("getaddresstxes",         &getaddresstxes),
