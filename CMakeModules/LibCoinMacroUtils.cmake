...
MACRO(LINK_CORELIB_DEFAULT CORELIB_NAME)
    ...
    TARGET_LINK_LIBRARIES(${CORELIB_NAME} ${ALL_GL_LIBRARIES})
    LINK_WITH_VARIABLES(${CORELIB_NAME})
    IF(LIBCOIN_SONAMES)
      SET_TARGET_PROPERTIES(${CORELIB_NAME} PROPERTIES VERSION ${LIBCOIN_VERSION} SOVERSION ${LIBCOIN_SOVERSION})
    ENDIF(LIBCOIN_SONAMES)
...
#    ENDFOREACH(LINKLIB)
    ...
    IF(MSVC  AND LIBCOIN_MSVC_VERSIONED_DLL)
        #when using full path name to specify linkage, it seems that already linked libs must be specified
            TARGET_LINK_LIBRARIES(${TARGET_TARGETNAME} ${ALL_GL_LIBRARIES})
    ENDIF(MSVC AND LIBCOIN_MSVC_VERSIONED_DLL)
...
MACRO(SETUP_LIBRARY LIB_NAME)
    ...
    ELSE()
        ...
        IF(TARGET_EXTERNAL_LIBRARIES)
            TARGET_LINK_LIBRARIES(${LIB_NAME} ${TARGET_EXTERNAL_LIBRARIES})
        ENDIF()
        IF(TARGET_LIBRARIES_VARS)
...
MACRO(SETUP_PLUGIN PLUGIN_NAME)
    ...
    ## plugins gets put in libcoin by default
    IF(${ARGC} GREATER 1)
      SET(PACKAGE_COMPONENT libcoin-${ARGV1})
    ELSE(${ARGC} GREATER 1)
      SET(PACKAGE_COMPONENT libcoin)
    ENDIF(${ARGC} GREATER 1)

    # Add the VisualStudio versioning info    
    SET(TARGET_SRC ${TARGET_SRC} ${LIBCOIN_VERSIONINFO_RC})
...
#the installation path are differentiated for win32 that install in bib versus other architecture that install in lib${LIB_POSTFIX}/${LIBCOIN_PLUGINS}
    IF(WIN32)
        INSTALL(TARGETS ${TARGET_TARGETNAME} 
            ...
            ARCHIVE DESTINATION lib/${LIBCOIN_PLUGINS} COMPONENT libcoin-dev
            LIBRARY DESTINATION bin/${LIBCOIN_PLUGINS} COMPONENT ${PACKAGE_COMPONENT})
    ELSE(WIN32)
        INSTALL(TARGETS ${TARGET_TARGETNAME}
            RUNTIME DESTINATION bin COMPONENT ${PACKAGE_COMPONENT}
            ARCHIVE DESTINATION lib${LIB_POSTFIX}/${LIBCOIN_PLUGINS} COMPONENT libcoin-dev
...
MACRO(SETUP_EXE IS_COMMANDLINE_APP)
    ...
    ELSE(${IS_COMMANDLINE_APP})
        ...
        IF(APPLE)
            # SET(MACOSX_BUNDLE_LONG_VERSION_STRING "${LIBCOIN_MAJOR_VERSION}.${LIBCOIN_MINOR_VERSION}.${LIBCOIN_PATCH_VERSION}")
            # Short Version is the "marketing version". It is the version
            # the user sees in an information panel.
            SET(MACOSX_BUNDLE_SHORT_VERSION_STRING "${LIBCOIN_MAJOR_VERSION}.${LIBCOIN_MINOR_VERSION}.${LIBCOIN_PATCH_VERSION}")
            # Bundle version is the version the OS looks at.
            SET(MACOSX_BUNDLE_BUNDLE_VERSION "${LIBCOIN_MAJOR_VERSION}.${LIBCOIN_MINOR_VERSION}.${LIBCOIN_PATCH_VERSION}")
            SET(MACOSX_BUNDLE_GUI_IDENTIFIER "org.libcoin.${TARGET_TARGETNAME}" )
...
MACRO(SETUP_APPLICATION APPLICATION_NAME)
        ...
        ELSE(APPLE)
            INSTALL(TARGETS ${TARGET_TARGETNAME} RUNTIME DESTINATION bin COMPONENT libcoin  )
        ENDIF(APPLE)
...
MACRO(SETUP_TEST TEST_NAME)
        ...
        IF(APPLE)
            INSTALL(TARGETS ${TARGET_TARGETNAME} RUNTIME DESTINATION share/libcoin/bin BUNDLE DESTINATION share/libcoin/bin )
        ELSE(APPLE)
            INSTALL(TARGETS ${TARGET_TARGETNAME} RUNTIME DESTINATION share/libcoin/bin COMPONENT libcoin-tests )
        ENDIF(APPLE)
...
MACRO(HANDLE_MSVC_DLL)
        ...
        ELSE(${ARGC} GREATER 1)
                SET(LIB_SOVERSION ${LIBCOIN_SOVERSION})
        ENDIF(${ARGC} GREATER 1)
